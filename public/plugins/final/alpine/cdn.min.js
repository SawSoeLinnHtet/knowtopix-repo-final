(() => {
    ...
    var H = class extends Error {
        ...
        $ = class extends Error {
            constructor(t) {
                super(`#${t} was not found in the current document.`),
    ...
    var f = {
            ...
            next(e, t) {
                let i = e.indexOf(t);
                if (i !== -1)
                    return this.teleportTo(this.teleportBack(e[i + 1]));
            },
            teleportTo(e) {
    ...
    function j(e, t, i) {
        ...
        function U(n, a, d) {
            ...
            for (; s; ) {
                ...
                let I = (p) =>
                        p.nodeType === 8 && p.textContent === " __BLOCK__ ",
                    K = (p) =>
                        p.nodeType === 8 && p.textContent === " __ENDBLOCK__ ";
                if (I(s) && I(u)) {
                    let p = [],
                        B,
                        T = 0;
    ...
    async function O(e, t, i, r = !0) {
        ...
        let l = await ce(e);
        if (
            (l.ok ? o("ajax:success", l) : o("ajax:error", l),
            o("ajax:after", l),
            !l.html)
        )
            return;
        ...
        t = t.map((c) => {
            ...
            if (!g) {
                ...
                if (
                    (c.hasAttribute("x-sync") ||
                        console.warn(
                            `Target #${c.id} not found in AJAX response.`
                        ),
                    l.ok)
                )
    ...
    async function ce({ method: e, action: t, body: i, referrer: r }) {
        ...
        if (e === "GET") {
            ...
            (l = (c) => X(t, (g) => g.resolve(c))),
                (m = (c) => X(t, (g) => g.reject(c)));
        }
    ...
    function Ee(e, t) {
        let i = Array.from(e.entries()).filter(
            ([r, o]) => o !== "" || o !== null
        );
        if (i.length) {
            let r = t.split("#");
    ...
    function Z(e) {
        Q(window),
            ...
            e.magic("ajax", (t) => (i, r = {}) => {
                ...
                let m = null;
                if (r.body)
                    if (r.body instanceof HTMLFormElement)
                        m = new FormData(r.body);
                    else {
                        m = new FormData();
